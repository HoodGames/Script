...
local Load = function()
    ...
    local Settings, FOVCircle, FOVCircleOutline, Offset = Environment.Settings, Environment.FOVCircle, Environment.FOVCircleOutline
...
function Environment.Blacklist(self, Username)
    assert(self, "Blacklist: Missing self")
    assert(Username, "Blacklist: Missing Username")
    Username = FixUsername(Username)
    assert(self, "Blacklist: User not found")
    self.Blacklisted[#self.Blacklisted + 1] = Username
end

function Environment.Whitelist(self, Username)
    assert(self, "Whitelist: Missing self")
    assert(Username, "Whitelist: Missing Username")
    Username = FixUsername(Username)
    assert(Username, "Whitelist: User not found")
    local Index = tablefind(self.Blacklisted, Username)
    assert(Index, "Whitelist: User is not blacklisted")
