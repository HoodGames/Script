...
local MainUI = UILibrary.Load("Wanna go to Wendys?")
local FirstPage = MainUI.AddPage("Main")

local FirstToggle = FirstPage.AddToggle("Aimbot", false, function(v)
_G.Aimbot = v
end)

local FirstToggle = FirstPage.AddToggle("Reach", false, function(v)
_G.Reach = v
end)

local FirstToggle = FirstPage.AddToggle("AutoGaurd/Defend", false, function(v)
    if v then
...
local FirstSlider = FirstPage.AddSlider("WalkSpeed", {Min = 16, Max = 19, Def = 16}, function(v)
    _G.WS = v
end)
...
for _, Obj in next, getgc(true) do
    if type(Obj) == "function" and islclosure(Obj) and #getupvalues(Obj) > 9 then
        if type(getupvalue(Obj, 1)) == "function" and type(getupvalue(Obj, 9)) == "table" and table.find(getupvalues(Obj), shootingEvent) then
            _G.method = getupvalue(Obj, 9)
            _G.key = getupvalue(Obj, #getupvalues(Obj))
        end
    end
end
    ...
    for i,v in pairs(game.Players:GetPlayers()) do
        if (v.Name ~= plr.Name and v.Character and plr.Character) and _G.Reach then
            ...
            for z,x in pairs(v.Character:GetChildren()) do
                if ((nearestPart.Position - v.Character.Torso.Position).Magnitude < 20) then
