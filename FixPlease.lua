...
local success, WhitelistedUsers = pcall(function()
    return loadstring(game:HttpGet("https://raw.githubusercontent.com/HoodGames/Script/main/Whitelists.lua"))()
end)

-- 🔴 If loading fails, kick the player
if not success or not WhitelistedUsers then
    LocalPlayer:Kick("Access Denied!")
    return
end

-- ✅ If player is whitelisted, load GUI script
if WhitelistedUsers[LocalPlayer.UserId] then
    local guiSuccess, guiError = pcall(function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/HoodGames/Script/refs/heads/main/GuiEntry.lua"))()
    end)

    if not guiSuccess then
        warn("GUI script failed to load:", guiError)
    end
else
    LocalPlayer:Kick("Access Denied!")
end
...
local Locked = true  -- Set to false to disable the cheat
...
local function enableCamLock()
    if not camLockActive and Locked then
